...
def test_secretary_send_to_unassigned_reviewer_blocked(
    client, db_session, make_user, make_reviewer, make_building
):
    sec, sec_h = make_user(UserRole.SECRETARY)
    sec.group_no = 1
    ...
    res = client.post(
        "/api/notifications/send", headers=sec_h,
        json={"recipient_ids": [unassigned_user.id], "title": "t", "message": "m"},
    )

    assert res.status_code == 403
    assert res.json()["detail"]["invalid_recipient_ids"] == [unassigned_user.id]
...
def test_secretary_review_reminder_excludes_unassigned_reviewer(
    client, db_session, make_user, make_reviewer, make_building
):
    sec, sec_h = make_user(UserRole.SECRETARY)
    sec.group_no = 1
    _, unassigned_rev, _ = make_reviewer()
    unassigned_rev.group_no = None
...
def test_secretary_list_notifications_excludes_unassigned_recipient(
    client, db_session, make_user, make_reviewer, make_building
):
    sec, sec_h, _, other_user, _, _ = _setup(
        make_user, make_reviewer, make_building, db_session
    )
    unassigned_user, unassigned_rev, _ = make_reviewer()
    unassigned_rev.group_no = None
    ...
    db_session.add(NotificationLog(
        recipient_id=unassigned_user.id, channel="kakao", template_type="t",
        title="unassigned-recipient", message="m",
    ...
    assert res.status_code == 200
    titles = [it["title"] for it in res.json()["items"]]
    assert "unassigned-recipient" not in titles
